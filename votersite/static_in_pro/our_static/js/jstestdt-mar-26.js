...
// AJAX for posting
function tag_voter() {
    console.log("in tag_voter trying to post! - about to set the html fields"); // sanity check

    console.log('printing the values in the html fields....');
    console.log('#post-notes-desc ', $('#post-notes-desc').val());
    console.log('#post-team-member-username', $('#post-team-member-username').val());
    console.log('#post-campaign-username',$('#post-campaign-username').val());
    console.log('#post-notes-status',$('#post-notes-status').val());
    console.log('#post-campaign-action',$('#post-campaign-action').val());
    ...
    var notes_desc = $('#post-notes-desc').val(), // value entered by user
        team_member_username =  $('#post-team-member-username').val(), // return the value bound to this hidden field
        campaign_username = $('#post-campaign-username').val(), // return the value bound to this hidden field
        campaign_action = $('#post-campaign-action').val(), // value entered by user
        voter_id = selected_voter_data[selected_voter_row].voter_id, // return the value entered into this input field
        notes_status = $('#post-notes-status').val(); // return the value bound to this hidden field
        
    console.log('voter_id being passed = ', voter_id);
    ...
    $.ajax({
        ...
        success : function(json) {
            $('#post-notes-desc').val(''); // remove the value from the input
            //var message = ['Voter tag successful for voterid', voter_id, 'notes ', notes_desc];
            var message = [json.success, 
                            //'voter id: '+json.voter_id,   
                            //'Added: '+json.notes_desc,  
                            'by team member : '+json.team_member_username +
                            ' for campaign: '+json.campaign_username,
                            'Additional notes for this voter can be entered below'];
            ...
            document.getElementById('post_context').innerHTML = showMessage(message);
...
$('#voters tbody').on('click', 'tr', function() {
    //$('.voterdetails').on('click', function() {  // this didn't work
    //$('#post_context').val('');
    document.getElementById('post_context').innerHTML = ''; // clear out the context
...
        else { // the item selected was not already showing the child so...
    ...
            this_row
            ...
            var context_message = ['Selected: ', 
                                    selected_voter_data[selected_voter_row].name_and_age,
                                    selected_voter_data[selected_voter_row].street_number + " " +
                                    selected_voter_data[selected_voter_row].street_name + " " +
                                    selected_voter_data[selected_voter_row].unit];
            //document.getElementById('post_context').appendChild(showMessage(context_message)); 

            // cag20160409 set the voter-id-val in the field.. hmmmmm
            //$('#post-voter-id').val(selected_voter_data[selected_voter_row].voter_id);
            document.getElementById('post_context').innerHTML = showMessage(context_message);
