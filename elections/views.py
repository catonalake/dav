...
from .models import Fullvs, Fullvstats, CampaignAssignment, Fulldav, VoterNotes, CampaignTeamMember
from .forms import ContactForm, SignUpForm, VoterNotesForm
...
def home(request): 
    ...
    if request.user.is_authenticated():
        sitevisitor = request.user.username # capitalize the first letter of the username
        #print (sitevisitor)
    ...
    if (request.method == "POST"):
        ...
        if form.is_valid():
            ...
            contact_message = """
            %s: %s via %s
            """% (form_first_name, form_emailaddr, from_email)
...
def vstats(request):
    ...
    if request.user.is_authenticated():
        sitevisitor = request.user.username
        print(sitevisitor, ' is authenticated')
        # get the team members record and their associated campaign assignment
        try:
            campaign_team_record = CampaignTeamMember.objects.filter(team_member_username=sitevisitor).\
                filter(status=active_campaign_status)
        except:
            return HttpResponse("Your id is not active at this time :)")

        # cag20161010 - remove this as table is set to allow only  one user (user is unique key)
        ct_ctr = 0
        for ct in campaign_team_record:
            ct_ctr +=1
            print ('campaign username %s set up for team member  %s' %(ct.campaign_username, ct.team_member_username))
            campaign_username = ct.campaign_username

        if ct_ctr > 1:
            return HttpResponse("Your id can only be set up with one team")

        # sanity check: xxxxxxxxxxxxxxxxxxxx
        #print ('campaign_team_record.campaign_username = ',campaign_team_record.campaign_username)

        try:
            campaign = get_list_or_404(CampaignAssignment.objects.\
                filter(campaign_username=campaign_username).\
                filter(status=active_campaign_status).\
                filter(campaign_type=request_controller))
        except:
            return HttpResponse("Error in campaign assignment lookup.  You must have an active campaign set up to access this  :)")
    ...
    if campaign_username == request.user.username:
        notes_status = "A"
    else:
        notes_status = "P"

    data = {'team_member_username': request.user.username,
            'campaign_username': campaign_username, 
            'notes_status': notes_status, # this is overridden for everyone right now..
    }
    form = VoterNotesForm(data)
...
def tag_voter(request): 
    ...
    if (request.method == "POST"): 
        ...
        team_member_username =  request.POST.get('team_member_username', '')
        campaign_username =  request.POST.get('campaign_username', '')
        campaign_action =  request.POST.get('campaign_action', '')
        notes_status =  request.POST.get('notes_status', '')

        #campaign_action =  request.POST.get('campaign_action', '')
        # if campaign_action is true.... do stuff
        # sanity check
        print('...notes_desc = ', notes_desc)
        print('...voter_id = ', voter_id)
        print('...team_member_username = ', team_member_username)
        print('...campaign_username = ', campaign_username)
        print('...campaign_action = ', campaign_action)
        print('...notes_status = ', notes_status)


        form = VoterNotesForm(request.POST or None) # need or None or else shows up 
        if form.is_valid():
            #notes_status = form.clean_notes_status(campaign_username, team_member_username)
            notes_status = form.clean_notes_status()
            date_added = form.clean_date_added() # this is just to set the date...

            instance = form.save(commit=False)
            instance.save()
            success = "Note Added Successfully"
            failedText = "Note Added Successfully"
        else:
            failedText = "form is not valid"
            success = "form is not valid"
    else:
        form = VoterNotesForm() 
        success = "not a post request"
        failedText = "not a post request"
...
def reviewvoternotes(request):
    ...
    notes_array = []
    camp_array = []
    query_information = "Voter Notes"

    if url_path == 'pendingnotes':
        request_controller   = url_path
        list_description = "Pending Notes"
    elif url_path == 'approvednotes':
        request_controller = url_path
        list_description = "Approved Notes"
    elif url_path == 'followupnotes': # will only exclude rejected notes
        request_controller = url_path
        list_description = "Notes Requiring Voter Follow Up"
    else:
        return HttpResponse("voter notes error - invalid url path  :)" + url_path)
    ...
    if request.user.is_authenticated():
        sitevisitor = request.user.username
        print(sitevisitor, ' is authenticated')
        # get the team member record and their associated campaign assignment
        try:
            campaign_team_record = CampaignTeamMember.objects.filter(team_member_username=sitevisitor).\
                filter(status=active_campaign_status)
        except:
        ...
        # cag20161010 - rework this as table is set to allow only  one user (user is unique key)
        # needed right now to get the first item
        ct_ctr = 0
        for ct in campaign_team_record:
            ct_ctr +=1
            print ('campaign username %s set up for team member  %s' %(ct.campaign_username, ct.team_member_username))
        ...
        # return the campaign that is set up specifically for this user (i.e. team member user = campaign user )
        # there should only be one right now since we requre sitevisitor 
        try: 
            campaign = get_list_or_404(CampaignAssignment.objects.\
                filter(campaign_username=sitevisitor).\
                filter(status=active_campaign_status))
        except:
            return HttpResponse("Error in campaign assignment lookup.  You must be an active campaign manager to access this  :)")
        # get the campaign information for this person
    else:
        return HttpResponse("You must be logged in to see this content  :)")
        # if they are not authenticated, remind them to log in..  
    ...
    for c in campaign:
        ctr +=1
        c_text = c.campaign_type +" "+ c.campaign_type_value.title()
        camp_array.append(c_text)
        campaign_username = c.campaign_username # these are all the same as campaign_username is the same as sitvisitor...- not needed
        #print (c_text)

    town_list = tuple(camp_array)


    if request_controller == 'pendingnotes':
        try:
            vnotes_table = VoterNotes.objects.\
                                filter(notes_status="P").\
                                filter(campaign_username=sitevisitor).\
                                order_by('-date_added')
        except:
            query_information = "No Pending " + query_information + " found for campaign " + campaign_username
            print (query_information + ' eventually just display on the page returned')
            #return HttpResponse("Error retrieving homes in city/ town campaign  :)")
    elif request_controller == 'approvednotes':
        try:
            vnotes_table = VoterNotes.objects.\
                                filter(notes_status="A").\
                                filter(campaign_username=sitevisitor).\
                                order_by('-date_added')
        except:
            query_information = "No Approved " + query_information + " found for campaign " + campaign_username
            print (query_information + ' eventually just display on the page returned')
            #return HttpResponse("Error retrieving homes in city/ town campaign  :)")
    elif request_controller == 'followupnotes':
        try:
            vnotes_table = VoterNotes.objects.\
                                filter(campaign_action="F").\
                                filter(campaign_username=sitevisitor).\
                                order_by('-date_added')
        except:
            query_information = "No Follow Up " + query_information + " found for campaign " + campaign_username
            print (query_information + ' eventually just display on the page returned')
            #return HttpResponse("Error retrieving homes in city/ town campaign  :)")
    else:
        return HttpResponse("Error processing request  :)")

    #form = VoterNotesForm(data) ? not sure what form will be used for this...
    context = {'vnotes_table': vnotes_table,
               'sitevisitor': sitevisitor,
               'town_list': town_list,
               'query_information ': query_information,
               'request_controller': request_controller,
    ...
    return render(request, 'elections/voternotes.html', context)
# end reviewvoternotes
...
def campaign_dashboard(request):
    ...
    print('url path = ' + resolver_match.url_name)
    camp_array = []
    query_information = "Campaign Dashboard"

    if url_path == 'campaign_dashboard':
        request_controller   = url_path
        list_description = "Campaign Dashboard"
    else:
        return HttpResponse("Invalid url path  :)" + url_path)
    ...
    if request.user.is_authenticated():
        ...
        # cag20161010 - rework this as table is set to allow only  one user (user is unique key)
        # needed right now to get the first item without referencing the first element using [0]
        ct_ctr = 0
        for ct in campaign_team_record:
            ct_ctr +=1
            print ('campaign username %s set up for team member  %s' %(ct.campaign_username, ct.team_member_username))
        ...
        if ct_ctr > 1:
            return HttpResponse("Your id can only be set up with one team")
            # we will have to adjust this down the road...  

        # return all the types set up for the campaign id
        # there can be only one team member assignment right now - 
        # but there can be many types/ districts for the specific campaign
        try: 
            campaign_type_list = get_list_or_404(CampaignAssignment.objects.\
                filter(campaign_username=campaign_username).\
                filter(status=active_campaign_status))
        except:
            return HttpResponse("Oops - No active campaigns found for your id  :)")
        # get the campaign information for this person
    else:
        return HttpResponse("You must be logged in to see this content  :)")
        # if they are not authenticated, remind them to log in..  
    ...
    house_array = []
    ctr = 0
    for c in campaign_type_list:
        ctr +=1
        c_text = c.campaign_username+ " "+ c.campaign_type.title() +" "+ c.campaign_type_value.title()
        if c.campaign_type == "CITY":
            city_array.append(c)
        elif c.campaign_type == "SENATEDISTRICTS":
            senate_array.append(c)
        elif c.campaign_type == "HOUSEDISTRICTS":
            house_array.append(c)

        camp_array.append(c.campaign_type.title() +" "+ c.campaign_type_value.title())
        print (c_text)
    ...
    context = {'campaign_type_list': campaign_type_list,
               'sitevisitor': sitevisitor,
               'town_list': town_list,
               'query_information ': query_information,
               'request_controller': request_controller,
               ...
               'senate_array': senate_array,
               # 'form': form,
               }
    return render(request, 'elections/campaigndashboard.html', context)
    # end 
...
def detailjson(request):
    ...
    voterinfo = get_object_or_404(Fulldav, pk=voter_id)
    context = {
    'first_name': voterinfo.first_name,
    'last_name': voterinfo.last_name,
    'street_number': voterinfo.street_number,
    'street_name': voterinfo.street_name,
    'unit':voterinfo.unit,
    'city':voterinfo.city,
    'postal_city':voterinfo.postal_city,
    'street_name_2': voterinfo.street_name_2,
    'state_rep_district': voterinfo.state_rep_district,
    'state_senate_district': voterinfo.state_senate_district,
    'precinct': voterinfo.precinct,
    'zip_code': voterinfo.zip_code,
    'age': voterinfo.age,
    'current_party': voterinfo.current_party,
    'sex': voterinfo.sex,
    }
    #voterjson = serializers.serialize("json", get_object_or_404(Fulldav, pk=voter_id))
